...
<body>
    ...
    <main class="container mt-3">
        ...
        <h1 class="text-center">Chỉnh sửa truyện tiểu thuyết</h1>
        <?php
        include '../partials/connect_db.php';
        $query = "SELECT * FROM truyen WHERE id={$_GET['id']}";
        $sth = $pdo->query($query);
        $row = $sth->fetch();
        echo "<form action=\"update.php?id={$row['id']}\" name=\"truyen\" method=\"POST\" class=\"w-50 m-auto mt-4\">
                <div class=\"input-group mb-3 row\">
                <span for=\"tentruyen\" class=\"input-group-text col-3 d-flex justify-content-center\">Tên truyện</span>
                <input type=\"text\" class=\"form-control col-9\" name=\"tentruyen\" value=\"{$row['tentruyen']}\">
                
            </div>
            <div class=\"input-group mb-3 row\">
                <span for=\"tacgia\" class=\"input-group-text col-3 d-flex justify-content-center\">Tác giả</span>
                <input type=\"text\" class=\"form-control col-9\" name=\"tacgia\" value=\"{$row['tacgia']}\">
            </div>
            <div class=\"input-group mb-3 row\">
                <span for=\"theloai\" class=\"input-group-text col-3 d-flex justify-content-center\">Thể loại</span>
                <input type=\"text\" class=\"form-control col-9\" name=\"theloai\" value=\"{$row['theloai']}\">
            </div>
            <div class=\"input-group mb-3 row\">
                <span for=\"img\" class=\"input-group-text col-3 d-flex justify-content-center\">Hình Poster</span>
                <input type=\"text\" class=\"form-control col-9\" name=\"img\" value=\"{$row['img']}\">
            </div>
            <div class=\"input-group mb-3 row\">
                <span for=\"noidung\" class=\"input-group-text col-3 d-flex justify-content-center\">Nội dung</span>
                <textarea class=\"form-control col-9\" spellcheck=\"false\" rows=\"10\" name=\"noidung\">{$row['noidung']}</textarea>
            </div>
            
            <button class=\"btn btn-primary mb-5\" type=\"submit\">Cập nhật truyện</button>
            </form>";
